...
*/
const ob = [{ flavor: 'Chocolate', count: 36 }, { flavor: 'Vanilla', count: 210 }];
const flavor ='Vanilla';
const flavorDeleter = flavor => (
  {
    type:'DELETE_FLAVOR',
    flavor
  }
...
function appReducer (state, action) {
  switch (action.type) {
    case 'DELETE_FLAVOR' :
      return state.filter(state=>state.flavor!=action.flavor);
    default:
      return state;
  }
}
console.log(appReducer(ob,flavorDeleter('Vanilla')));
