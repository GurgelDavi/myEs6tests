...
/* The following code is the equivalent of the bellow but chained
const results = musicData.filter(
  album => (album.name.length + album.artist.length) <=25
);
const totalAlbumSales = results.reduce((acumulator,currentValue)=> {
  return acumulator+currentValue.sales;
},0)
*/
const totalAlbumSales = musicData.filter(
  album => (album.name.length + album.artist.length) <=25
