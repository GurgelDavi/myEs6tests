...
/*
 uniqueflavors = data.map(d => (d.favoriteIceCreams))
 uniqueflavors = [].concat(...uniqueflavors)
 uniqueflavors = uniqueflavors.filter(function(item,pos){return uniqueflavors.indexOf(item) == pos })
 */
iceCreamTotals = data.reduce((acumulator, currentValue) => {
  currentValue.favoriteIceCreams.forEach(flavor=>{
    (flavor in acumulator) ? acumulator[flavor] ++ : acumulator[flavor] = 1
  })
  return acumulator
}, {})
